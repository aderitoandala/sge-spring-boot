...
public class AproveitamentoService {
	...
	public AproveitamentoService(AproveitamentoRepository aproveitamentoRepository, AlunoRepository alunoRepository,
	...
	public AproveitamentoResponseDTO listarPorId(Long id) {
		return AproveitamentoMapper.toDTO(aproveitamentoRepository.findById(id)
				.orElseThrow(() -> new RecursoNaoEncontradoException("Aproveitamento não encontrado")));
	}

	public AproveitamentoResponseDTO listarPorAlunoESemestre(Long alunoId, Integer semestre) {
		return AproveitamentoMapper.toDTO(aproveitamentoRepository.findByAlunoIdAndSemestre(alunoId, semestre)
				.orElseThrow(() -> new RecursoNaoEncontradoException("Aproveitamento não encontrado")));
	}
	...
	public AproveitamentoResponseDTO criar(AproveitamentoRequestDTO dto) {
		...
		DisciplinaEntity disciplina = disciplinaRepository.findById(dto.getDisciplinaId())
				.orElseThrow(() -> new RecursoNaoEncontradoException("Disciplina não encontrada"));
		if (aproveitamentoRepository.existsByAlunoIdAndDisciplinaIdAndSemestre(dto.getAlunoId(), dto.getDisciplinaId(),
				dto.getSemestre())) {
	...
	public AproveitamentoResponseDTO actualizar(Long id, AproveitamentoRequestDTO dto) {
		...
		AproveitamentoEntity ap = aproveitamentoRepository.findById(id)
				.orElseThrow(() -> new RecursoNaoEncontradoException("Aproveitamento não encontrado"));
		AlunoEntity aluno = alunoRepository.findById(dto.getAlunoId())
				.orElseThrow(() -> new RecursoNaoEncontradoException("Aluno não encontrado"));
		DisciplinaEntity disciplina = disciplinaRepository.findById(dto.getDisciplinaId())
